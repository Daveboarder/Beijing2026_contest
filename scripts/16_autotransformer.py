"""Benchmark or submit the depth-profile Transformer autoencoder + CLS MLP.

uv run --extra cnn python scripts/16_autotransformer.py benchmark --device cuda --tag initial
uv run --extra cnn python scripts/16_autotransformer.py predict --recipe PATH --device cuda
"""
...
def _preparation(section, depth_section):
    """Prefer autotransformer prep keys; fall back to depth_transformer defaults."""
    return {
...
def benchmark(cfg, args):
    section = cfg.get("autotransformer", {})
    depth_section = cfg.get("depth_transformer", {})
    preparation = _preparation(section, depth_section)
    ...
    params = dict(
        ...
        d_model=section.get("d_model", 64), n_layers=section.get("n_layers", 2),
        n_heads=section.get("n_heads", 4), ff_dim=section.get("ff_dim", 128),
        dropout=section.get("dropout", 0.1),
        lambda_recon=section.get("lambda_recon", 1.0),
        epochs=args.epochs or section.get("epochs", 100),
        batch_size=section.get("batch_size", 8),
        patience=section.get("patience", 15),
        lr=section.get("lr", 0.0003),
    ...
    root = cfg.results_dir / "autotransformer" / args.tag
    ...
    for repeat, folds in enumerate(splits):
        ...
        for seed in seeds:
            ...
            for fold, (tr, te) in enumerate(folds):
                model = AutoDepthClassifier(
                    **params, device=args.device, random_state=seed)
                model.fit(X[tr], y[tr])
                ...
                print(f"autotransformer repeat={repeat} seed={seed} fold={fold} "
                      f"selected_epochs={model.best_epoch_}", flush=True)
            probabilities.append(p)
            for i, sid in enumerate(ids):
                all_predictions.append(dict(
                    model="autotransformer", repeat=repeat, seed=str(seed),
        ...
        for i, sid in enumerate(ids):
            all_predictions.append(dict(
                model="autotransformer", repeat=repeat, seed="ensemble",
                sample_id=sid, y_true=y[i],
                **{f"p{c}": ensemble[i, j] for j, c in enumerate(classes)}))

    row = dict(model="autotransformer",
    ...
    summary = [row]
    recipe = dict(
        config=cfg.raw, preparation=preparation, model_params=params, seeds=seeds,
    ...
    (root / "recipe_autotransformer.json").write_text(json.dumps(recipe, indent=2),
                                                      encoding="utf-8")
...
def predict(args):
    ...
    for seed in recipe["seeds"]:
        ...
        model = AutoDepthClassifier(**params, val_fraction=0,
...
def main():
    ...
    pred.add_argument("--output", default="submissions/autotransformer.csv")
